...
/*
 * Adding a filter to add images behind each item in the navegation menu
 * */
function nav_menu_background_img( $item_output, $item, $depth, $args ) {
		if($args->menu_id=='top-menu')
		{
			$item_output = '<div class="menu-item-child-container">'. $item_output;
			# Verifying if the current menu item is related to the current page, so it will show the second 
			# background image of the menu item by default
			if(in_array('current-menu-item',$item->classes))
			{
				$item_output .= '<img class="nav-image nav-image-default" src="'. get_parent_theme_file_uri( '/assets/images/'. strtolower(str_replace(" ","-",$item->title)) .'-1.svg'). '" hidden></img>';
				$item_output .= '<img class="nav-image nav-image-hover" src="'. get_parent_theme_file_uri( '/assets/images/'. strtolower(str_replace(" ","-",$item->title)) .'-2.svg'). '"></img>';	
			}
			else
			{
				$item_output .= '<img class="nav-image nav-image-default" src="'. get_parent_theme_file_uri( '/assets/images/'. strtolower(str_replace(" ","-",$item->title)) .'-1.svg'). '"></img>';
				$item_output .= '<img class="nav-image nav-image-hover" src="'. get_parent_theme_file_uri( '/assets/images/'. strtolower(str_replace(" ","-",$item->title)) .'-2.svg'). '" hidden></img>';
			}
			$item_output .= '</div>';
		}	
    return $item_output;
	}
	add_filter( 'walker_nav_menu_start_el', 'nav_menu_background_img', 10, 4 );